...
/**
 * CalDbAuthManager represents an authorization manager that stores authorization
 * information in database with special performance improvements for UWebCal.
 *
 ...
 * @author André Mekkawi <uwebcal@andremekkawi.com>
 */
class CalDbAuthManager extends CDbAuthManager
{
	/**
	 * @var boolean whether or not the table uses foreign keys for cascading updates/deletes. 
	 */
	...
	{
		parent::init();
		$this->_usingSqlite = $this->_usingSqlite || !$useForeignKeys;
	}
	...
	{
		// TODO: Pass $itemName to getAuthAssignments
	...
	protected function checkAccessRecursive($itemName,$userId,$params,$assignments)
	{
		if(($item=$this->getAuthItem($itemName))===null)
			return false;
		Yii::trace('Checking permission "'.$item->getName().'"','system.web.auth.CalDbAuthManager');
	...
	public function getAuthItems($type=null,$userId=null)
	{
		...
		else if($type===null)
		{
			...
			$conditions = 'name=itemname AND userid=:userid';
		...
		else
		{
			...
			$conditions = 'name=itemname AND type=:type AND userid=:userid';
			$params = array(
				':type'=>$type,
				':userid'=>$userId
			);
		...
		if ($type !== null && count($typeParts = explode(':', $type, 2)) == 2) {
			if (strlen($conditions))
				$conditions .= ' AND ';
			
			if ($typeParts[0] == '*') {
				$conditions .= 'calendarid IS NULL';
			}
			else {
				$conditions .= 'calendarid = :calendarid';
				$params[':calendarid'] = $typeParts[0];
			}
		}
	...
	// TODO: Add clearAll for a specific calendar
	...
	// TODO: Add clearAuthAssignments for a specific calendar
	...
	protected function getDbConnection()
	{
		if($this->db!==null)
			...
			throw new CException(Yii::t('yii','CalDbAuthManager.connectionID "{id}" is invalid. Please make sure it refers to the ID of a CDbConnection application component.',
